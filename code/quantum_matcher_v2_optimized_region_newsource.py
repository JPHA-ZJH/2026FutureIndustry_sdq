...
# 粗筛必须覆盖全部 core 词，而不能只用 STRONG_CONTEXT_TERMS。
# 这样 coarse_screen=True 只跳过必然不可能进入正式量子池的记录，
# 与逐条全量评分的正式识别口径保持一致，同时显著减少运行时间。
CORE_CANDIDATE_TERMS = []
for _rule in QUANTUM_RULES:
    if _rule.get("match_type", "core") == "core":
        CORE_CANDIDATE_TERMS.extend(_rule.get("terms_cn", []))
        CORE_CANDIDATE_TERMS.extend(_rule.get("terms_en", []))
CORE_CANDIDATE_PATTERNS = compile_terms(sorted(set(CORE_CANDIDATE_TERMS)))

COARSE_REGEX = re.compile("|".join([p.pattern for _, p in CORE_CANDIDATE_PATTERNS]), flags=re.IGNORECASE)
...
def make_text_series(
    ...
    en_abs_col: Optional[str] = "摘要 (英文)",
    extra_text_cols: Optional[Sequence[str]] = None,
) -> pd.Series:
    """Combine only columns that actually exist; None means that language field is absent."""
    cols = [c for c in [cn_abs_col, en_abs_col] if c]
    if extra_text_cols:
        cols.extend([c for c in extra_text_cols if c])
...
def _existing_region_cols(
    ...
    if isinstance(region_col, str):
        ...
        cols = list(region_col)
    return [c for c in cols if c in df.columns]


def _join_unique_semicolon(x: pd.Series) -> str:
    vals = []
    for s in x.dropna().astype(str):
        vals.extend([v.strip() for v in s.split("；") if v.strip()])
    return "；".join(sorted(set(vals)))


def _most_common_nonempty(x: pd.Series) -> str:
    vals = [str(i) for i in x.dropna() if str(i).strip()]
    return Counter(vals).most_common(1)[0][0] if vals else ""


def summarize_quantum_firms(
    ...
    """
    Aggregate already-tagged quantum patents.

    region_col may be one column or a list, e.g.
    ["第一申请人地区", "第一申请人城市"].
    """
    if quantum_patents.empty or firm_col not in quantum_patents.columns:
        return pd.DataFrame(), pd.DataFrame()
    if year_col not in quantum_patents.columns:
        raise KeyError(f"缺少年份列：{year_col}")

    data = quantum_patents.copy()
    ...
    if firm_type_col and firm_type_col in data.columns:
        year_named_aggs["first_applicant_types"] = (firm_type_col, _join_unique_semicolon)
        firm_named_aggs["first_applicant_types"] = (firm_type_col, _join_unique_semicolon)

    firm_year_quantum = (
        data.groupby(group_cols_year, dropna=False)
        .agg(**year_named_aggs)
        .reset_index()
    )

    firm_quantum = (
        data.groupby(group_cols_firm, dropna=False)
        .agg(**firm_named_aggs)
        .reset_index()
    )

    firm_main_position = (
        data.groupby(group_cols_firm, dropna=False)["chain_position"]
        .agg(_most_common_nonempty)
        .reset_index()
        .rename(columns={"chain_position": "main_chain_position"})
    )
    firm_main_category = (
        data.groupby(group_cols_firm, dropna=False)["main_category"]
        .agg(_most_common_nonempty)
        .reset_index()
    ...
    firm_quantum = (
        firm_quantum
        .merge(firm_main_position, on=group_cols_firm, how="left")
        .merge(firm_main_category, on=group_cols_firm, how="left")
    )
    ...
    return firm_year_quantum, firm_quantum
...
def tag_quantum_patents(
    ...
    include_timefreq: bool = False,
):
    """
    Returns: patent_tagged, quantum_patents, firm_year_quantum, firm_quantum.

    For the new source without English abstract, pass en_abs_col=None.
    region_col can be one column or several location columns.
    """
    data = df.copy()
    text_series = make_text_series(data, cn_abs_col, en_abs_col, extra_text_cols)

    if coarse_screen:
        # The core regex now contains every match_type='core' term, so this screen
        # does not alter the formal core/time-frequency identification result.
    ...
    for i, text in enumerate(text_series, start=1):
        ...
        if progress_every and (i % progress_every == 0 or i == total_n):
            ...
            print(
                f"已处理 {i:,}/{total_n:,} 条候选，占比 {i/total_n:.2%}，"
                f"已用 {elapsed/60:.1f} 分钟，预计剩余 {remain/60:.1f} 分钟"
            )

    score_columns = list(score_one_patent_text("").keys())
    scores = pd.DataFrame(results, index=data.index, columns=score_columns)
    ...
    patent_tagged["is_quantum_patent"] = (
        patent_tagged["relevance"].isin(relevant_labels).astype(int)
    )
