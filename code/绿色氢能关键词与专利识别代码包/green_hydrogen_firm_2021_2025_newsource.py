# -*- coding: utf-8 -*-
"""从2021—2025年中国全量专利CSV识别绿色氢能专利并汇总第一申请人。"""
import re
import sys
import time
...
OUTPUT_ROOT = Path(r"F:\01科研\横向课题\未来产业识别\绿色氢能\data")
START_YEAR, END_YEAR = 2021, 2025
CHUNK_SIZE = 100000
NROWS_PER_YEAR = None
CSV_ENCODING = None
ALLOWED_FIRST_APPLICANT_TYPES = None  # 如只保留企业，改为 {"企业"}
MIN_GREEN_HYDROGEN_SCORE = 1
TARGET_COLS = ["专利名称", "专利类型", "申请人", "申请人类型", "申请人地区", "申请人城市",
               "公开公告年份", "IPC主分类号", "摘要文本", "主权项内容", "被引证次数"]

if str(SCRIPT_DIR) not in sys.path: sys.path.insert(0, str(SCRIPT_DIR))
from green_hydrogen_matcher_v1_optimized_region_newsource import (  # noqa: E402
    export_keyword_dictionary, summarize_green_hydrogen_firms, tag_green_hydrogen_patents)
...
for file_year in range(START_YEAR, END_YEAR + 1):
    ...
    for chunk_no, chunk in enumerate(iter_chunks(path), 1):
        ...
        _, matched, _, _ = tag_green_hydrogen_patents(
            ...
            progress_every=10000, min_score=MIN_GREEN_HYDROGEN_SCORE)
        matched["来源文件年份"], matched["来源分块"] = file_year, chunk_no
        parts.append(matched); matched_total += len(matched)
        print(f"{file_year}年第{chunk_no}块：发明授权 {len(chunk):,}，绿色氢能 {len(matched):,}")
    print(f"{file_year}年完成：原始 {raw_total:,}，发明授权 {grant_total:,}，绿色氢能 {matched_total:,}")
...
if parts:
    ...
    firm_year, firm = summarize_green_hydrogen_firms(
        patents, "第一申请人", "year", ["第一申请人地区", "第一申请人城市"], "第一申请人类型")
else:
    patents = firm_year = firm = pd.DataFrame()

paths = [OUTPUT_ROOT / "green_hydrogen_patents_2021_2025_newsource.csv",
         OUTPUT_ROOT / "firm_year_green_hydrogen_2021_2025_newsource.csv",
         OUTPUT_ROOT / "firm_green_hydrogen_2021_2025_newsource.csv",
         OUTPUT_ROOT / "green_hydrogen_keyword_dictionary.csv"]
