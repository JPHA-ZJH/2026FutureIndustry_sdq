# -*- coding: utf-8 -*-
"""从2021—2025年中国全量专利CSV识别6G专利并汇总第一申请人。"""
import re
import sys
import time
...
OUTPUT_ROOT = Path(r"F:\01科研\横向课题\未来产业识别\6G\data")
...
from sixg_matcher_v1_optimized_region_newsource import (  # noqa: E402
    export_keyword_dictionary, summarize_sixg_firms, tag_sixg_patents)
...
def add_first_fields(df):
    data = df.copy()
    for source, target in {"申请人": "第一申请人", "申请人类型": "第一申请人类型",
                           "申请人地区": "第一申请人地区", "申请人城市": "第一申请人城市"}.items():
        data[target] = data[source].map(first_item)
    return data
...
def check_columns(path):
    ...
    columns = pd.read_csv(path, **kwargs).columns
    missing = [column for column in TARGET_COLS if column not in columns]
    if missing:
        raise KeyError(f"文件 {path.name} 缺少变量：{missing}")
...
for file_year in range(START_YEAR, END_YEAR + 1):
    ...
    check_columns(path)
    raw_total = grant_total = matched_total = 0
    print(f"\n开始处理：{path}")
    for chunk_no, chunk in enumerate(iter_chunks(path), 1):
        ...
        chunk["year"] = years.fillna(file_year).astype("Int64")
        chunk = add_first_fields(chunk)
        if ALLOWED_FIRST_APPLICANT_TYPES is not None:
            chunk = chunk[chunk["第一申请人类型"].isin(ALLOWED_FIRST_APPLICANT_TYPES)].copy()
        _, matched, _, _ = tag_sixg_patents(
            chunk, cn_abs_col="摘要文本", en_abs_col=None,
            firm_col="第一申请人", year_col="year",
            region_col=["第一申请人地区", "第一申请人城市"],
            firm_type_col="第一申请人类型",
            extra_text_cols=["专利名称", "主权项内容"],
            split_firms=False, coarse_screen=True,
            progress_every=10000, min_score=MIN_SIXG_SCORE)
        matched["来源文件年份"], matched["来源分块"] = file_year, chunk_no
        parts.append(matched)
        matched_total += len(matched)
        print(f"{file_year}年第{chunk_no}块：发明授权 {len(chunk):,}，6G {len(matched):,}")
    print(f"{file_year}年完成：原始 {raw_total:,}，发明授权 {grant_total:,}，6G {matched_total:,}")

if parts:
    patents = pd.concat(parts, ignore_index=True)
    dedup_cols = [column for column in ["专利名称", "申请人", "公开公告年份", "IPC主分类号", "摘要文本", "主权项内容"]
                  if column in patents.columns]
    patents = patents.drop_duplicates(subset=dedup_cols).copy()
    firm_year, firm = summarize_sixg_firms(
        patents, "第一申请人", "year", ["第一申请人地区", "第一申请人城市"], "第一申请人类型")
else:
    patents = firm_year = firm = pd.DataFrame()

paths = [
    OUTPUT_ROOT / "sixg_patents_2021_2025_newsource.csv",
    OUTPUT_ROOT / "firm_year_sixg_2021_2025_newsource.csv",
    OUTPUT_ROOT / "firm_sixg_2021_2025_newsource.csv",
    OUTPUT_ROOT / "sixg_keyword_dictionary.csv",
]
